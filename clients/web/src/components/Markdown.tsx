...
const chatgptCommand: commands.ICommand = {
  name: "ChatGPT",
  keyCommand: "chatgpt",
  ...
  execute: async (state, api) => {
    ...
    console.log("newSelectionRange", selectedText);
    const newText = await soloapi.textToText({
      text: selectedText,
      context: "Du ska skriva om texten i en mer dramatisk fantasy stil",
    });
    api.replaceSelection(newText.text);
    const selectionStart = state1.selection.start;
    const selectionEnd = selectionStart + newText.text.length;
    ...
    // );
    // console.log("replaceSelection", modifiedText.modifiedText);
  ...
  return (
    <div>
      <MDEditor
        height={700}
        value={markdown}
        commands={[...commands.getCommands(), chatgptCommand]}
